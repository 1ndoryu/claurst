...
struct Cli {
    ...
    /// LLM provider to use (default: freellmapi when available, otherwise anthropic). Examples: openai, google, ollama
...
async fn main() -> anyhow::Result<()> {
    ...
    let provider_was_explicit = config.provider.is_some();
    let model_was_explicit = config.model.is_some();
    let api_key_was_explicit = cli.api_key.is_some();
    freellmapi::bootstrap_if_needed(
        &cwd,
        &mut config,
        provider_was_explicit,
        model_was_explicit,
        api_key_was_explicit,
    )
    .await?;
